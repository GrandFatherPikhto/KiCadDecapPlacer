...
- Чтение пользовательских полей компонентов (например, `Role` для автоматического подбора ролей).
- Получение bounding box'ов объектов (для keepout и коллизий).
- Транзакционное обновление и создание объектов (via, перемещение компонентов).
...
- При чтении пользовательских полей через IPC важно, чтобы поле было добавлено в схеме (Eeschema) и перенесено на плату через **Update PCB from Schematic**. Поле `Role` используется в KiCadSpoke для автоматического подбора компонентов.
