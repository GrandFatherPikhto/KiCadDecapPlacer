...
"""
Реализует релаксацию (раздвижку) точек вдоль рядов, чтобы устранить коллизии между компонентами.
get_tangential_axis() – определяет, вдоль какой оси (X или Y) происходит раздвижка, исходя из нормали к границе зоны.
relax_1d() – раздвигает точки вдоль одной оси, группируя их в кластеры и центрируя каждый кластер, чтобы сохранить общее положение.
relax_positions() – высокоуровневая обёртка, которая группирует точки по нормали и перпендикулярной координате (т.е. по одному ряду), затем применяет relax_1d() к каждому ряду.
"""
